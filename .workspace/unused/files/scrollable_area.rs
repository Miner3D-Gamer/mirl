...
impl ScrollableArea {
    ...
    pub fn scroll(&mut self, by: (f32, f32), vertical: bool) {
        if vertical {
            self.offset_x = by.0.mul_add(self.scroll_multiplier_x, self.offset_x);
            self.offset_y = by.1.mul_add(self.scroll_multiplier_y, self.offset_y);
        } else if !self.horizontal_context_switch_multipliers {
            self.offset_x = by.1.mul_add(self.scroll_multiplier_y, self.offset_x);
            self.offset_y = by.0.mul_add(self.scroll_multiplier_x, self.offset_y);
        } else {
            self.offset_x = by.1.mul_add(self.scroll_multiplier_x, self.offset_x);
            self.offset_y = by.0.mul_add(self.scroll_multiplier_y, self.offset_y);
        }
